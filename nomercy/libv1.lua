...
if not isfolder("NoMercy") then
    makefolder("NoMercy")
end
if not isfolder("NoMercy/cfg") then
    makefolder("NoMercy/cfg")
end
...
local ConfigFile = "NoMercy/cfg/@" .. gameName .. ".json"
...
local NoMercy = {}
function NoMercy:MakeNotify(NotifyConfig)
    local NotifyConfig = NotifyConfig or {}
    NotifyConfig.Title = NotifyConfig.Title or "NoMercy"
...
function nomercy(msg, delay, color, title, desc)
    return NoMercy:MakeNotify({
        Title = title or "NoMercy",
...
function NoMercy:Window(GuiConfig)
    GuiConfig              = GuiConfig or {}
    GuiConfig.Title        = GuiConfig.Title or "NoMercy"
    GuiConfig.Footer       = GuiConfig.Footer or "@NoMercy"
    ...
    local NoMercyF = Instance.new("ScreenGui");
    local DropShadowHolder = Instance.new("Frame");
    local DropShadow = Instance.new("ImageLabel");
    ...
    NoMercyF.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    NoMercyF.Name = "NoMercyF"
    NoMercyF.ResetOnSpawn = false
    NoMercyF.Parent = game:GetService("CoreGui")
    ...
    DropShadowHolder.ZIndex = 0
    DropShadowHolder.Name = "DropShadowHolder"
    DropShadowHolder.Parent = NoMercyF

    DropShadowHolder.Position = UDim2.new(0, (NoMercyF.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0,
        (NoMercyF.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("NoMercyF") then
            NoMercyF:Destroy()
        end
    end
    ...
    Close.Activated:Connect(function()
        ...
        Yes.MouseButton1Click:Connect(function()
            if NoMercyF then NoMercyF:Destroy() end
