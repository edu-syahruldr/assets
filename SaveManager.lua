...
local SaveManager = {} do
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddButton("Copy Config", function()
            ...
            if not success then
                self.Library:Notify("Failed to read config file!", 2)
                return
            end

            local clipboardFunc = setclipboard or toclipboard or 
                (Clipboard and Clipboard.set) or (syn and syn.write_clipboard)
            if clipboardFunc then
                pcall(function() clipboardFunc(content) end)
                self.Library:Notify("Config copied to clipboard!", 2)
            else
                print("[SaveManager] Config JSON:")
                print(content)
                self.Library:Notify("Check console (F9) for JSON", 2)
            end
        end)
        ...
        section:AddButton("Load from Input", function()
            ...
            local success, decoded = pcall(HttpService.JSONDecode, HttpService, importJson)
            if not success or type(decoded) ~= "table" then
                self.Library:Notify("Invalid JSON format!", 2)
                return
            end

            if not decoded.objects or type(decoded.objects) ~= "table" then
                self.Library:Notify("Invalid config format!", 2)
                return
            end
            ...
            self.Library:Notify("Loaded " .. count .. " settings from JSON!", 2)
        end)

        -- self:LoadAutoloadConfig()
        self:SetIgnoreIndexes({ "SaveManager_ConfigList", "SaveManager_ConfigName", "SaveManager_ImportJson" })
    end
